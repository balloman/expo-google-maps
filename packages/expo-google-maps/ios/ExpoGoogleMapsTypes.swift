...
struct Coordinate: Record {
  ...
  /// Converts this Coordinate to a Core Location coordinate.
  /// - Returns: A `CLLocationCoordinate2D` constructed from this coordinate's `latitude` and `longitude`.
...
struct Camera: Record {
  ...
  /// Converts this Camera into a `GMSCameraPosition` suitable for Google Maps.
  /// - Returns: A `GMSCameraPosition` configured with this camera's center coordinate, `zoom`, `bearing` (defaults to 0 if `nil`), and `viewingAngle` (defaults to 0 if `nil`).
...
struct Insets: Record {
  ...
  /// Creates a `UIEdgeInsets` from the inset fields, using 0 for any nil values.
  /// - Returns: A `UIEdgeInsets` whose `top`, `left`, `bottom`, and `right` are taken from the corresponding fields or 0 if the field is `nil`.
